...
import { decodeJid } from './simple.js'

function bind(conn) {
  if (conn.__storeInitialized) return
  conn.__storeInitialized = true

  let chats = null
  let listening = false

  function ensureStore() {
    if (!chats) chats = Object.create(null)

    if (!listening) {
      listening = true

      conn.ev.on('groups.update', updates => {
        if (!updates || !chats) return
  ...
  Object.defineProperty(conn, 'chats', {
    configurable: true,
    enumerable: true,
    get() {
      ensureStore()
